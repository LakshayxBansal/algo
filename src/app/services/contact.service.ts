  ...
  try {
    ...
    for(let i=1;i<=10;++i){
      ...
      if (key in customMasterListData) {
        query += `,cfd.${key} as ${key}_id,${customMasterListData[key].table_name}.${customMasterListData[key].field} as ${key}`;
      }
      else{
        query+=`,cfd.${key}`
      }
    ...
    for(const key in customMasterListData){
      query+=`left outer join ${customMasterListData[key].table_name}_master on cfd.${key}=${customMasterListData[key].table_name}`
    }    
