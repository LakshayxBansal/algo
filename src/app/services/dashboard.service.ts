...
export async function getOpenEnquiriesCountDb(dbName: string) {
  try {
    const result = await excuteQuery({
      host: dbName,
      query:
        "select count(*) as total from enquiry_ledger_tran lt where datediff(lt.date, now())=0;",
      values: [],
    });
...
export async function getEnquiriesOverviewDb(dbName: string) {
  try {
    const result = await excuteQuery({
      host: dbName,
      query:
        "SELECT em.name, ROW_NUMBER() OVER () as id, \
...
export async function getUnassignedEnquiriesDb(dbName: string) {
  try {
    const result = await excuteQuery({
      host: dbName,
      query:
        "SELECT * FROM enquiry_ledger_tran WHERE status_id=1 AND allocated_to IS NULL",
      values: [],
    });
...
export async function getClosedEnquiriesCountDb(dbName: string) {
  try {
    const result = await excuteQuery({
      host: dbName,
      query:
        "select DATE_FORMAT(CURDATE() - INTERVAL 5 MONTH, '%Y-%m-01') as since, count(*) as count from enquiry_ledger_tran lt\
...
export async function getAverageAgeDb(dbName: string) {
  try {
    const result = await excuteQuery({
      host: dbName,
      query:
        "SELECT ROUND(AVG(DATEDIFF(lt.modified_on, lt.date))) AS age, DATE_FORMAT(CURDATE() - INTERVAL 5 MONTH, '%Y-%m-01') as since\
                ...
                AND lt.date BETWEEN DATE_FORMAT(CURDATE() - INTERVAL 5 MONTH, '%Y-%m-01')\
