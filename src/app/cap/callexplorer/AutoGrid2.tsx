...
export default function AutoGrid() {
    ...
    const handleFilterChangeSubStatus = () => {
        const filteredRows = filterValueState["subStatus"]
            ? rows.filter((row) =>
                row.subStatus
                    .toLowerCase()
                    .includes(filterValueState["subStatus"]?.name.toLowerCase())
            )
            : row1;
    ...
    const handleFilterChangeCategory = () => {
        const filteredRows = filterValueState["callCategory"]
            ? rows.filter((row) =>
                row.callCategory
                    .toLowerCase()
                    .includes(filterValueState["callCategory"]?.name.toLowerCase())
            )
            : row1;
        setRows(filteredRows); // Set filtered rows using setRows
    ...
    const handleFilterChangeArea = () => {
        const filteredRows = filterValueState["area"]
            ? rows.filter((row) =>
                row.area
                    .toLowerCase()
                    .includes(filterValueState["area"]?.name.toLowerCase())
            )
            : row1;
        setRows(filteredRows); // Set filtered rows using setRows
    ...
    const handleFilterChangeExec = () => {
        ...
        if (filterType === "allocated") {
            filteredRows = filterValueState["executive"]
                ? rows.filter((row) =>
                    row.executive
                        .toLowerCase()
                        .includes(filterValueState["executive"]?.name.toLowerCase())
                )
                : row1;
    ...
    const handleFilterChangeNextAction = () => {
        const filteredRows = filterValueState["nextAction"]
            ? rows.filter((row) =>
                row.nextAction
                    .toLowerCase()
                    .includes(filterValueState["nextAction"].name.toLowerCase())
            )
            : row1;
        setRows(filteredRows);
    ...
    const handleFilterChangeCallStatus = () => {
        if (selectedStatus && callFilter == "0") {
            const filteredRows = row1.filter(
                (row) => row.callStatus === selectedStatus
            );
            setRows(filteredRows);
        }
    ...
    const handleFilterChangeDate = () => {
        ...
        const filteredRows = rows.filter((row) => {
            ...
            const initial = filterValueState.actionDate?.initial
                ? new Date(filterValueState.actionDate?.initial).toLocaleDateString(
                    "en-CA"
                )
                : null;

            const final = filterValueState.actionDate?.final
                ? new Date(filterValueState.actionDate?.final).toLocaleDateString(
                    "en-CA"
                )
                : null;
    ...
    const CustomColor = (props: customCol) => {
        ...
        if (props.row.callStatus === "Open") {
            if (props.row.executive === null) {
                color = "blue";
            } else {
                color = "purple";
            }
        } else {
            if (props.row.subStatus === "Success") {
                color = "green";
            } else {
                color = "red";
            }
    ...
    const column1: GridColDef[] = [
                    ...
                    >
                        <MenuItem>
                            <AutocompleteDB
                                ...
                                onChange={(e, val, s) =>
                                    handleFilterChange("callCategory", val)
                                }
        ...
        },
        {
            ...
            renderHeader: () => (
                <Box>
                    <OutlinedButton
                        sx={{ color: filterValueState.area ? "blue" : "black" }}
                        startIcon={
        ...
        },
        {
            ...
            renderHeader: () => (
                <Box>
                    <OutlinedButton
                        sx={{ color: filterType !== "reset" ? "blue" : "black" }}
                        startIcon={
        ...
        },
        {
            ...
            renderHeader: () => (
                <Box>
                    <OutlinedButton
                        sx={{ color: selectedStatus !== "" ? "blue" : "black" }}
                        startIcon={
        ...
        },
        {
            ...
            renderHeader: () => (
                <Box>
                    <OutlinedButton
                        sx={{ color: filterValueState.subStatus ? "blue" : "black" }}
                        startIcon={
        ...
        },
        {
            ...
            renderHeader: () => (
                <Box>
                    <OutlinedButton
                        sx={{ color: filterValueState.nextAction ? "blue" : "black" }}
                        startIcon={
            ...
            renderCell: (params) => <span>{params.row.nextAction}</span>,
        },
        {
            field: "actionDate",
            ...
            renderHeader: () => (
                <Box>
                    <OutlinedButton
                        sx={{ color: dateFilter !== "0" ? "blue" : "black" }}
                        startIcon={
    ...
    return (
        <Box sx={{ bgcolor: "#fceff3" }}>
            ...
            {selectedRow && (
                <Box>
                    {" "}
                    Call Details : {selectedRow.callNo} ({selectedRow.contactParty}
                    )(Org:)(Ledger:)
                </Box>
            )}
            <Paper
