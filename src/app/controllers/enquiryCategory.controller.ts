...
import { getEnquiryCategoryList, createEnquiryCategoryDb } from '../services/enquiryCategory.service';
...
export async function createEnquiryCategory(formData: FormData){
  let result;
    try {
    const session = await getSession();
    if (session) {
